...
contains
 ...
 !*********************************************************
  ...
  implicit none  
  real (kind(0.e0)), intent(in out)        :: A(:,:)
  logical, intent(out)                     :: logicFlag
  ! loop and temp vars
  integer                                  :: m, n
  integer                                  :: i, j, k
 ...
 ! Finds the X in AX = B. 
    ...
    implicit none
    real(kind(0.e0)), intent(in out)  :: A(:,:)
    real(kind(0.e0)), intent(in out)  :: B(:)
    real(kind(0.e0)), intent(out)     :: X(:)
    ! loop and temp vars
    integer                               :: m, n
    real (kind(0.e0))                     :: tempSum
 ...
 ! After backsub, the least squares problem is solved.
    ...
    implicit none
    real(kind(0.e0)), intent(in)               :: A(:,:)
    real(kind(0.e0)), intent(out)              :: Vandermonde(:,:)
    real(kind(0.e0)), intent(out)              :: y(:)
 ...
 subroutine outerProd(u, v, A)
    ...
    implicit none
    real(kind(0.e0)), intent(in)         :: u(:)
    real(kind(0.e0)), intent(in)         :: v(:)
    real(kind(0.e0)), intent(out)        :: A(:,:)    
    ! loop and temp vars
    integer                              :: m,n 
    integer                              :: i,j
 ...
 subroutine householderQR(A, Q)
    ...
    implicit none
    real(kind(0.e0)), intent(in out)     :: A(:,:)
    real(kind(0.e0)), intent(out)        :: Q(:,:)
    ! loop and temp vars
    real(kind(0.e0)), allocatable        :: Vmat(:,:) 
    real(kind(0.e0)), allocatable        :: v(:)
    real(kind(0.e0)), allocatable        :: s(:)
    integer                              :: m, n !dims
    integer                              :: i, j, k
    real(kind(0.e0))                     :: norm
    real(kind(0.e0))                     :: normTwo 
    real(kind(0.e0)), allocatable        :: tempMat(:,:)
    real(kind(0.e0)), allocatable        :: vt(:) 
    ! the following declarations are used in creation of Q
    real(kind(0.e0)), allocatable        :: Id(:,:)
    real(kind(0.e0)), allocatable        :: H(:,:)
 ...
 subroutine backsubQR(R, b, x)
    ...
    ! x a solution vector
    implicit none
    real(kind(0.e0)), intent(in)   :: R(:,:)
    real(kind(0.e0)), intent(in)   :: b(:)
    real(kind(0.e0)), intent(out)  :: x(:) 
    ! temp and loop vars
    real(kind(0.e0))               :: tempSum
    integer                        :: i, k, m
